...
from rept_utilities import toimage, save_image, save_clue, miniimagenet_downloader
...
class Dataset:
    def __init__(self, split, version, TR, vallim, index, input_shape, num_channels):
        ...
        n_class = 0
        if split == 'train':
            metatrain_folder = pickle.load(open('mini-imagenet-cache-train.pkl', 'rb'))
            x_data = metatrain_folder['image_data']
            y_vec = metatrain_folder['class_dict']
            print(y_vec.keys())
            for y in y_vec.keys():
                n_class = n_class + 1
                for i in y_vec[y]:
                    y_data.append(n_class)
        else:
            metatest_folder = pickle.load(open('mini-imagenet-cache-test.pkl', 'rb'))
            print(metatest_folder.keys())        
            x_data = metatest_folder['image_data']
            y_vec = metatest_folder['class_dict']
            print(y_vec.keys())
            for y in y_vec.keys():
                n_class = n_class + 1
                for i in y_vec[y]:
        ...
        print(" ** Miniimagenet doesn't have to be shuffled...")
